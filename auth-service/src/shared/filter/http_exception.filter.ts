  ...
  private readonly logger = new Logger(HttpExceptionFilter.name);
    ...
    if (exception instanceof HttpException) {
      ...
      if (
        ...
        this.logger.error(
          `[JWT Error] ${errorResponse['message']} | Path: ${request.method} ${request.url}`,
        );
      }

      // Log toàn bộ exception (bao gồm stack trace nếu có)
      this.logger.error(
        `[HttpException] ${JSON.stringify(errorResponse)} | Path: ${request.method} ${request.url} | Stack: ${exception.stack}`,
      );
